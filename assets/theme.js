(function ($) {
  ...
  var halo = {
            ...
            if (url != null && url != undefined) {
              $.ajax({
                ...
                data: {
                  ...
                  constraint:
                    "limit=" + limit + "+layout=" + layout + "+sectionId=" + sectionId + "+imageRatio=" + image_ratio + "+swipe=" + swipe + "+hasCountdown=" + hasCountdown,
                },
                beforeSend: function () {
          ...
          .on("transitionend.toggleCloseMenu", () => {
            if (!$body.hasClass("menu_open")) {
              ...
              if (!$(".header-top--wrapper .header-top--right .customer-service-text").length) {
                if ($(".header").hasClass("header-03")) {
                  $("#navigation-mobile .site-nav-mobile.nav-account .customer-service-text").insertBefore(
    ...
    initMobileMenuDefault: function (url) {
      ...
      $(".list-menu-loading").remove();
      menuMobile.append(
        `<nav class="header__inline-menu" data-navigation role="navigation" style="${style != undefined ? style : ""}">${nav.html() != undefined ? nav.html() : ""}</nav>`
      );
            ...
            tabLink.off("click").on("click", function (e) {
              ...
              if (!$(this).hasClass("active")) {
                ...
                if (!curTabContent.hasClass("loaded"))
                  halo.doAjaxProductTabs(curTab.data("href"), curTabContent.find(".loading"), curTabContent.find(".products-load"), $block.attr("sectionid"), limit);
      ...
      $.ajax({
        ...
        success: function (data) {
          const $productGrid = showMoreButton.closest(".tab-content").length
            ...
            : showMoreButton.closest(".halo-block-content").find(".products-grid");
    ...
    buildRecommendationBlock: function () {
      ...
      const handleIntersection = (entries, observer) => {
        ...
        if (
          $this.innerHTML.trim() != "" &&
          !recommendationsContainer.classList.contains("product-recommendations-loading") &&
    ...
    productBlockScroller: function (wrapper) {
      ...
      let innerCursor = cursorWrapper.querySelector(".products-cursor__inner"),
        ...
        itemNotTween = wrapper
          .get(0)
          ?.querySelectorAll(".card-product__group, .card-action, .card-title, .card-swatch, .variants-popup, .card-compare, .card-quickview, .card-wishlist, .list-product-tabs"),
        clientX,
        clientY,
        scrollerBox,
        cursorSide = null,
      ...
      $doc.on("click", ".arrow-icon-scroll", (event) => {
        ...
        const headerHeight =
            window.innerWidth > 1024
              ? document.querySelectorAll("sticky-header")[0]?.getBoundingClientRect().height
              : document.querySelector("sticky-header-mobile")?.getBoundingClientRect().height || 0,
